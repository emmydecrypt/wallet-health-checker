 ...
 */
async function transfer(agent, amount, recipient, tokenAddress) {
    ...
    try {
        ...
        if (!tokenAddress) {
            ...
            const formattedAmount = (0, index_1.formatPharos)(amount, 18);
            if (!formattedAmount) {
        ...
        const decimals = await (0, index_1.getTokenDecimals)(agent, tokenAddress);
        if (decimals === null || decimals === undefined) {
            throw new Error(`Failed to retrieve token decimals for contract: ${tokenAddress}`);
        }
        const formattedAmount = (0, index_1.formatPharos)(amount, decimals);
        if (!formattedAmount) {
            throw new Error("Failed to format token amount");
        }
        const tokenBalance = await (0, index_1.getERC20Balance)(agent, tokenAddress);
